...
export function myAtoi(str: string) {
    const reg = /^([-\+]?[0-9]+).*/
    ...
    if(groups){
       targetNum = Number(groups[1])
       if(Number.isNaN(targetNum)) return 0
    }

    if(targetNum >= INT_MAX) return INT_MAX

    if(targetNum <= INT_MIN) return INT_MIN
